     ...
     const bayarLimit = (sender, amount) => {
		...
		case 'antilink':
					...
					if (args[0] == 'on') {
						...
						reply(`Ativando o modo anti-link com sucesso`)
					} else if (args[0] == 'off') {
...
/*********END PISAH MENU***********/
         ...
         case 'lirik':
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
				reply(mess.wait)
					if (args.length < 1) return reply('cade o texto tio?')
               ...
               if (isLimit(sender)) return reply(limits.limitend(pushname2))
                ...
                if (isLimit(sender)) return reply(limits.limitend(pushname2))
					...
					client.sendMessage(from, pok, image, { quoted: mek , caption: 'Miau🐈'})
					await limitAdd(sender) 
					break 
			...
			case 'google':
               ...
               if (isBanned) return reply(mess.only.benned)    
				...
				default:
                  ...
                  reply(`Desculpe man *${pushname2}*, Comando*${prefix}${command}* Não listado dentro do menu digite*${prefix}menu!*`)
                  }
